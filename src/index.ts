const NODE_FLAG_IN_QUEUE = 0b01;
const NODE_FLAG_DIRTY    = 0b10;
...
const NODE_PREV_OFFSET = 4;
const NODE_NEXT_OFFSET = 5;
const NODE_SUB_HEAD_OFFSET = 6;
const NODE_SUB_TAIL_OFFSET = 7;
const NODE_CLEANUP_OFFSET = 8;
const NODE_SIZE = 9;
...
function node(
  ...
  nodeMemory[nodePtr + NODE_PREV_OFFSET] = NULL_REPR;
  nodeMemory[nodePtr + NODE_NEXT_OFFSET] = NULL_REPR;
  nodeMemory[nodePtr + NODE_SUB_HEAD_OFFSET] = NULL_REPR;
  nodeMemory[nodePtr + NODE_SUB_TAIL_OFFSET] = NULL_REPR;
...
function enqueueNode(nodePtr: number) {
  ...
  if (queueHead === undefined) {
    ...
    nodeMemory[queueTail! + NODE_NEXT_OFFSET] = nodePtr;
    nodeMemory[nodePtr + NODE_PREV_OFFSET] = queueTail!;
    queueTail = nodePtr;
  }
}
...
function dequeueNode(nodePtr: number) {
  ...
  nodeMemory[nodePtr + NODE_FLAGS_OFFSET] ^= NODE_FLAG_IN_QUEUE;
  let prevPtr = nodeMemory[nodePtr + NODE_PREV_OFFSET];
  let nextPtr = nodeMemory[nodePtr + NODE_NEXT_OFFSET];
  if (prevPtr !== NULL_REPR) {
    nodeMemory[prevPtr + NODE_NEXT_OFFSET] = nextPtr;
  }
  if (nextPtr !== NULL_REPR) {
    nodeMemory[nextPtr + NODE_PREV_OFFSET] = prevPtr;
  }
